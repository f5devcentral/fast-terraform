...
Now we want to add a custom HTTP monitor and a snat pool.
Now update your terraform main.tf file with the following:
...
Now we want to add a second virtual server or application in the same tenant.
We create a second main.tf file in app2 folder with the following:
